...
CORE_VERSION="$(grep -m1 '^version\s*=\s*"' crates/agentic-contract/Cargo.toml | sed -E 's/.*"([^"]+)".*/\1/')"
MCP_VERSION="$(grep -m1 '^version\s*=\s*"' crates/agentic-contract-mcp/Cargo.toml | sed -E 's/.*"([^"]+)".*/\1/')"
...
AgenticContract v${CORE_VERSION} (with MCP v${MCP_VERSION}) delivers production-grade policy engine improvements with clearer governance controls and measurable deployment value.

## Business Impact

This release lowers operational friction for teams adopting structured agent governance, improves reliability across policy enforcement boundaries, and reduces risk exposure from uncontrolled agent actions.
...
- https://github.com/agentralabs/agentic-contract/compare/v${CORE_VERSION}...HEAD
EOF
...
echo "=== Publishing AgenticContract paired crates to crates.io ==="
...
if [ "${DO_PUBLISH}" = true ]; then
  ...
  gh release create "v${CORE_VERSION}" \
    --title "AgenticContract v${CORE_VERSION}" \
