...
policy_out="$(run_acon --path "$acon_file" policy add "No unreviewed deploys" --action deny --scope global)"
assert_contains "$policy_out" "Created policy" "policy add"
policy_out2="$(run_acon --path "$acon_file" policy add "Allow read-only queries" --action allow --scope session)"
assert_contains "$policy_out2" "Created policy" "policy add session"

echo "[3/8] Policy enforcement check"
check_out="$(run_acon --path "$acon_file" policy check allow --scope session)"
assert_contains "$check_out" "Allow" "policy check"

echo "[4/8] Risk limit lifecycle"
limit_out="$(run_acon --path "$acon_file" limit set "API calls per minute" --type rate --max 100)"
assert_contains "$limit_out" "Created risk limit" "limit set"
limit_list="$(run_acon --path "$acon_file" limit list)"
assert_contains "$limit_list" "Rate" "limit list"

echo "[5/8] Approval workflow"
rule_out="$(run_acon --path "$acon_file" approval rule "Deploy approval" --pattern "deploy:*")"
assert_contains "$rule_out" "Created approval rule" "approval rule"
# Extract full UUID from rule output
rule_id="$(printf '%s' "$rule_out" | grep -oE '[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}' | head -1)"
[ -n "$rule_id" ] || fail "Could not extract rule ID from: $rule_out"
req_out="$(run_acon --path "$acon_file" approval request "Deploy v1.2" --rule-id "$rule_id" --requestor dev-agent)"
assert_contains "$req_out" "Created approval request" "approval request"

echo "[6/8] Obligation tracking"
obligation_out="$(run_acon --path "$acon_file" obligation add "Complete audit before release" --description "Full security audit required" --deadline "2099-12-31T00:00:00Z")"
...
violation_out="$(run_acon --path "$acon_file" violation report "Unauthorized API access" --severity critical --actor rogue-agent)"
assert_contains "$violation_out" "Violation reported" "violation report"
