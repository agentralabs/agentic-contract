#!/bin/bash
# AgenticContract — one-liner install script
...
#
# What it does:
#   1. Downloads agentic-contract-mcp binary to ~/.local/bin/
...
# ── Constants ──────────────────────────────────────────────────────────
REPO="agentralabs/agentic-contract"
BINARY_NAME="agentic-contract-mcp"
SERVER_KEY="agentic-contract"
...
download_binary() {
    ...
    local asset_name="agentic-contract-${version_num}-${platform}.tar.gz"
    ...
    # Copy both binaries (acontract CLI + MCP server)
    cp "${tmpdir}"/agentic-contract-*/acontract "${INSTALL_DIR}/acontract" 2>/dev/null || true
    cp "${tmpdir}"/agentic-contract-*/${BINARY_NAME} "${INSTALL_DIR}/${BINARY_NAME}"
    chmod +x "${INSTALL_DIR}/acontract" 2>/dev/null || true
...
find_contract() {
    ...
    for candidate in \
        "\${ACONTRACT_FILE:-}" \
        "\${AGENTRA_ACONTRACT_PATH:-}" \
        "\${repo_root}/.agentra/\${repo_slug}.acontract" \
        "\${repo_root}/.contract.acontract" \
        "\$PWD/.contract.acontract" \
        "\$PWD/contract.acontract" \
        "\$HOME/.contract.acontract" \
        "\$HOME/.agentra/contract/default.acontract"; do
    ...
    printf '%s' "\${repo_root}/.agentra/\${repo_slug}.acontract"
}
...
for arg in "\${args[@]}"; do
    case "\$arg" in
        ...
        -c|--contract|--contract=*) has_contract=1 ;;
...
if [ "\$has_contract" -eq 0 ]; then
    contract_path="\$(find_contract || true)"
    if [ -n "\$contract_path" ]; then
        args=(--contract "\$contract_path" "\${args[@]}")
    fi
fi
...
print_terminal_server_help() {
    ...
    echo "  ${INSTALL_DIR}/acontract --help"
    echo "  ${INSTALL_DIR}/${BINARY_NAME}${SERVER_CHECK_CMD_SUFFIX}"
    echo "  (Ctrl+C to stop after startup check)"
}
...
print_post_install_next_steps() {
    ...
    if [ "$PROFILE" = "server" ]; then
        ...
        echo "  3. If artifacts were created on another machine, sync .acontract/.amem/.acb/.avis files to this server." >&3
        echo "  4. Start MCP with auth, connect clients, then restart clients." >&3
        echo "  5. Optional feedback: open https://github.com/agentralabs/agentic-contract/issues" >&3
    else
        echo "  2. Restart your MCP client/system so it reloads MCP config." >&3
        echo "  3. ${SERVER_KEY} now auto-detects local .acontract files at startup when available." >&3
...
main() {
    ...
    echo "AgenticContract Installer"
    echo "========================="
    echo ""
    ...
    finish_progress
    echo "Install complete: AgenticContract (${PROFILE})" >&3
    echo "" >&3
    echo "Done! Contract store defaults to .agentra/<project>.acontract" >&3
